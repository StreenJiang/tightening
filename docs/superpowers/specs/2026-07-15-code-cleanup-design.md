...
- `shouldSelfLoop` 局部变量
- `isTaskOk()` 私有方法（仅在 onCompleted 自循环分支中调用）
- `handleRestart()` 方法（唯一 `@Async` `@EventListener` 方法）
- `TaskCompletedEvent` 发布逻辑
...
`onFaulted` 回调删除 `selfLoopCounts.remove(taskId)` 行。

`trigger()` 方法保留 ProductTask / bolts / productCode / partsCode 参数，不再包含自循环逻辑。

删除 import：`@Async`、`@EventListener`、`ApplicationEventPublisher`、`TaskCompletedEvent`。

### 1.2 LifecycleEngineFactory.java

`createEngine()` 删除 `boolean shouldSelfLoop` 参数。`TaskContext.builder()` 不再设置 `.shouldSelfLoop(...)`。

### 1.3 TaskContext.java
...
### 1.5 TaskCompletedEvent.java

删除整个文件。该 record 仅用于自循环重启事件，发布者和消费者都在 TaskOrchestrator 内部。
...
### 2.3 ProductTask 使用 InspectionScope 枚举

`ProductTask.java` 和 `ProductTaskDTO.java` 的 `inspectionScope` 从 `Integer` 改为 `InspectionScope`。
...
- ~~**ReworkStatus**~~ — 已于 2026-07-17 删除。`task_record.is_rework` 字段当前用 Integer，无需枚举
...
| 项 | 文件 | 动作 |
|------|------|------|
| N1 | `TaskOrchestrator.java` | 删除自循环逻辑、publisher、isTaskOk、handleRestart |
| N1 | `LifecycleEngineFactory.java` | 删除 shouldSelfLoop 参数 |
| N1 | `TaskContext.java` | 删除 shouldSelfLoop 字段 |
| N1 | `LifecycleEngine.java` | 删除 setShouldSelfLoop(false) |
| N1 | `TaskCompletedEvent.java` | **删除** |
| G5 | `DeleteStatus.java` | **删除** |
| G5 | `TCPCommand.java` | **删除** |
| G5 | `InspectionScope.java` | 加 `@EnumValue`、`@JsonValue`、`@JsonCreator`，`fromCode` 改直接返回 |
| G5 | `ProductTask.java` | `Integer inspectionScope` → `InspectionScope inspectionScope` |
| G5 | `ProductTaskDTO.java` | `Integer inspectionScope` → `InspectionScope inspectionScope` |
