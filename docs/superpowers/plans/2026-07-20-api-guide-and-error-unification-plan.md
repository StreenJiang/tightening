...
**Architecture:** 新增 1 个 Markdown 文档；修改 2 个 Controller（TaskLifecycleController 5 处 badRequest→ok、DeviceController 4 处 API 改为 ApiResponse 包装）
...
- 不改 ProductTaskController（已经是正确模式）
...
### Task 2: 统一 TaskLifecycleController 错误返回格式
...
```java
// 第 64 行: "task already active" → ok()
// 之前:
return ResponseEntity.badRequest()
        .body(ApiResponse.fail("task already active: " + id));
// 之后:
return ResponseEntity.ok(
        ApiResponse.fail("task already active: " + id));

// 第 70 行: "task not found" → ok()
// 之前:
return ResponseEntity.badRequest()
        .body(ApiResponse.fail("task not found: " + id));
// 之后:
return ResponseEntity.ok(
        ApiResponse.fail("task not found: " + id));

// 第 74 行: "task has no bolts" → ok()
// 之前:
return ResponseEntity.badRequest()
        .body(ApiResponse.fail("task has no bolts: " + id));
// 之后:
return ResponseEntity.ok(
        ApiResponse.fail("task has no bolts: " + id));
```

- [ ] **Step 2: 修改 trigger 方法中第三处返回**

```java
// 第 80-81 行: "task already active" (orchestrator 返回 null 的情况)
// 之前:
if (engine == null) {
    return ResponseEntity.badRequest()
            .body(ApiResponse.fail("task already active: " + id));
...
if (engine.isEmpty()) {
    return ResponseEntity.ok(
            ApiResponse.fail("no active task: " + id));
}
```
...
git commit -m "refactor: unify error response format in TaskLifecycleController to HTTP 200"
```
