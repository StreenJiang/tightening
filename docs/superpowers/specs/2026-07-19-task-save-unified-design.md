...
```
POST   /api/tasks          → 新增任务
PUT    /api/tasks/{id}     → 更新任务（以路径 id 为准，忽略 DTO 中的 id）
```
...
| part | 类型 | 说明 |
|------|------|------|
| `dto` | application/json | `ProductTaskSaveDTO` |
...
| 原端点 | 融入目标 |
|--------|---------|
| `POST/DELETE /api/tasks/{id}/prerequisites/**` | SaveDTO.prerequisites |
| `POST/DELETE /api/tasks/{id}/inspection-bindings/**` | SaveDTO.inspectionBoundTaskIds |
| `POST/DELETE /api/tasks/{id}/barcode-rules/**` | SaveDTO.barcodeRules |
...
```java
// ProductTaskSaveDTO — 顶层 DTO
public class ProductTaskSaveDTO extends BaseDTO {
    // 基本字段（与 ProductTask 实体一致）
    private String name;
    ...
    // 子项列表
    private List<Long> inspectionBoundTaskIds;       // 点检绑定，全量替换
...
`ProductTaskService.saveTask(SaveDTO, imageMap)` — 一个 `@Transactional` 方法：

1. **保存 Task 基本字段** → 得到 taskId
2. **校验** `inspectionScope + binding`（`validateInspectionScope`）
3. **同步 inspectionBoundTaskIds** — 全量替换，批量查询目标任务后逐个校验
...
替换现有 `ProductTaskController` 中的 `POST /api/tasks` 和 `PUT /api/tasks/{id}` 为 multipart 版本。通过 `HttpServletRequest` 手动解析 `MultipartHttpServletRequest`，提取 `dto` 参数（JSON）和文件 parts。`asMultipart()` 共享守卫方法做类型安全检查。
...
`ProductTaskController` 中以下端点已移除：
...
| 3 | 重要 | `isInspectionTask` 三处内联 | 统一调用私有方法 |
...
| 7 | 次要 | saveTask 中 6 条注释重复代码语义 | 删除 |
