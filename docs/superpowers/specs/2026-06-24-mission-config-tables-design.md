...
> 设计日期: 2026-06-24
> 设计范围: 从旧系统 `aneng` MySQL 迁移 `product_mission`、`product_side`、`product_bolt` 三张表到当前项目 SQLite，优化字段命名和类型

## 概述

从旧系统 `aneng` MySQL 迁移四张表到当前项目 SQLite，构成拧紧任务的**配置层**——定义任务模板、产品面、螺栓点位、条码规则。与已有的运行时记录层（`mission_record` + `tightening_data`）通过 `product_mission_id` 关联。

```
product_mission (任务配置模板)
  ├── mission_prerequisite (新增 - 任务前置依赖)
  ├── inspection_mission_binding (新增 - 点检任务绑定)
  ├── bar_code_matching_rule (新增 - 条码匹配规则)
  ├── product_side (产品面 + 图片)
  │     └── product_bolt (螺栓点位)
  │           ├── bolt_device_binding (新增 - 螺栓设备关联)
  │           └── bolt_parts_barcode (新增 - 螺栓物料码关联)
  └── mission_record (已存在 - 运行时记录)
```
...
相比源表移除的字段：`pn_code`、`macs_id`、`is_challenge_mission`、`is_first_mission`、`challenge_mission_id`、`predecessor_mission_id`、`predecessor_part_mission_ids`、`user_id`。前置依赖逻辑拆入 `mission_prerequisite` 表。

### mission_prerequisite — 任务前置依赖
...
**后端校验规则**：
- SAME_TRACE / PARTS_TRACE 的 `prerequisite_mission_id` 必须指向 `is_inspection=0` 的任务
- INSPECTION_CHAIN 的 `prerequisite_mission_id` 必须指向 `is_inspection=1` 的任务
- 配置保存时做循环依赖检测

### inspection_mission_binding — 点检任务绑定
...
CREATE TABLE bar_code_matching_rule (
    id                    INTEGER PRIMARY KEY AUTOINCREMENT,
    name                  TEXT,
    product_mission_id    INTEGER,              -- FK → product_mission.id
...
**物料码绑定规则**：
- 任务级：通过 `product_mission_id` 绑定（`rule_type=PARTS_BARCODE`），任务激活前校验
...
| 表 | 索引列 | 说明 |
|---|---|---|
| `mission_prerequisite` | `mission_id` | 按任务查前置依赖 |
| `mission_prerequisite` | `prerequisite_mission_id` | 按前置任务反查 |
| `inspection_mission_binding` | `inspection_mission_id` | 按点检任务查绑定 |
| `inspection_mission_binding` | `bound_mission_id` | 按被点检任务反查 |
| `product_side` | `product_mission_id` | 按任务查面列表 |
| `product_bolt` | `product_side_id` | 按面查螺栓列表 |
| `bolt_device_binding` | `product_bolt_id` | 按螺栓查设备绑定 |
| `bolt_device_binding` | `device_id` | 按设备反查绑定 |
| `bar_code_matching_rule` | `product_mission_id` | 按任务查条码规则 |
| `bolt_parts_barcode` | `product_bolt_id` | 按螺栓查物料码 |
| `bolt_parts_barcode` | `bar_code_matching_rule_id` | 按条码规则反查 |

> 索引命名建议：`idx_表名_列名`，例如 `idx_mission_prerequisite_mission_id`。
...
- 时间字段使用 `TEXT`
- Entity 继承 `BaseEntity`，字段使用 `Integer` 而非枚举类型，与现有 `MissionRecord` 模式一致

## 迁移文件

| 序号 | 文件 | 内容 |
|---|---|---|
| V1.0.8 | `add_table_product_mission.sql` | product_mission + mission_prerequisite + inspection_mission_binding + bar_code_matching_rule |
...
```java
// com.tightening.entity.ProductMission.java
@Setter @Getter @ToString(callSuper = true) @EqualsAndHashCode(callSuper = true)
@Accessors(chain = true) @TableName("product_mission")
...
```java
// com.tightening.entity.MissionPrerequisite.java
@Setter @Getter @ToString(callSuper = true) @EqualsAndHashCode(callSuper = true)
@Accessors(chain = true) @TableName("mission_prerequisite")
...
```java
// com.tightening.entity.InspectionMissionBinding.java
@Setter @Getter @ToString(callSuper = true) @EqualsAndHashCode(callSuper = true)
@Accessors(chain = true) @TableName("inspection_mission_binding")
...
```java
// com.tightening.dto.ProductMissionDTO.java
@Setter @Getter @ToString(callSuper = true) @EqualsAndHashCode(callSuper = true)
...
```java
// com.tightening.mapper/ProductMissionMapper.java
@Mapper
public interface ProductMissionMapper extends BaseMapper<ProductMission> {}

@Mapper public interface MissionPrerequisiteMapper extends BaseMapper<MissionPrerequisite> {}
@Mapper public interface InspectionMissionBindingMapper extends BaseMapper<InspectionMissionBinding> {}
...
```java
// com.tightening.service/ProductMissionService.java
...
public class ProductMissionService extends ServiceImpl<ProductMissionMapper, ProductMission> {
    ...
    public void cascadeDelete(Long missionId) {
        // 批删模式：先收集 sideIds → 批量删除 bolts 及其关联 → 按 missionId 清空其他关联表 → 最后删除 mission
...
@Slf4j @Service public class MissionPrerequisiteService extends ServiceImpl<MissionPrerequisiteMapper, MissionPrerequisite> {}
@Slf4j @Service public class InspectionMissionBindingService extends ServiceImpl<InspectionMissionBindingMapper, InspectionMissionBinding> {}
...
public class ProductSideService extends ServiceImpl<ProductSideMapper, ProductSide> {
    ...
    /** 批量删除螺栓关联（给 ProductMissionService.cascadeDelete 调用），不修改当前 side */
...
> **级联删除模式**：`@Transactional` 仅标注在入口方法（`ProductMissionService.cascadeDelete`、`ProductSideService.cascadeDelete`、`ProductBoltService.cascadeDelete`）。内部的 `deleteBoltsBySideIds` 不标注 `@Transactional`，由调用方的事务管理（同一线程内 Propagation.REQUIRED）。
...
`MissionConfigValidator` 负责配置保存时的应用层校验：

| 规则 | 说明 |
|---|---|
| 前置类型约束 | SAME_TRACE/PARTS_TRACE 的 `prerequisite_mission_id` 必须指向 `is_inspection=0` 的任务；INSPECTION_CHAIN 必须指向 `is_inspection=1` 的任务 |
| 循环依赖检测 | 使用 **BFS**（`Deque` + `Set`）沿 `mission_prerequisite` 图广度遍历所有可达路径（不再限定单链），检测到环则拒绝保存 |
| key_char 长度 | `key_char.length()` 必须等于 `end_position - start_position + 1`（范围）或 1（单值），不匹配拒绝保存 |
| bolt_serial_num 唯一性 | 同一 `product_mission` 下所有 `bolt_serial_num` 不能重复，跨 side 也冲突 |
| 点检绑定约束 | `inspection_mission_binding.bound_mission_id` 必须指向 `is_inspection=0` 的任务 |
| 追溯码唯一 | 同一个 mission 最多一条 `rule_type=PRODUCT_TRACE` 的条码规则 |
| 图片大小 | 单张图片 ≤ 5MB，上传时校验 |
...
`ProductMissionService.cascadeDelete(missionId)` @Transactional 时的级联路径：
...
| `mission_prerequisite` | `lambdaUpdate().eq(missionId).or().eq(prerequisiteMissionId).remove()` |
| `inspection_mission_binding` | `lambdaUpdate().eq(inspectionMissionId).or().eq(boundMissionId).remove()` |
| `bar_code_matching_rule` | `lambdaUpdate().eq(productMissionId).remove()` |
...
|---|---|---|---|---|
| `ProductMissionController` | `controller/ProductMissionController.java` | `api/missions` | Mission CRUD（`GET` 列表、`GET /{id}` 详情、`POST` 创建、`PUT /{id}` 更新、`DELETE /{id}` 删除）、前置依赖管理（`POST /{missionId}/prerequisites`、`GET /{missionId}/prerequisites`、`DELETE /{missionId}/prerequisites/{id}`）、点检绑定管理（`POST /{inspectionMissionId}/inspection-bindings`、`GET /{missionId}/inspection-bindings`、`DELETE /{missionId}/inspection-bindings/{id}`）、条码规则管理（`POST /{missionId}/barcode-rules`、`GET /{missionId}/barcode-rules`、`DELETE /{missionId}/barcode-rules/{id}`） |
| `ProductSideController` | `controller/ProductSideController.java` | `api/sides` | Side CRUD（`GET ?missionId=` 列表、`GET /{id}` 详情、`POST` 创建、`PUT /{id}` 更新、`DELETE /{id}` 删除）、图片下载（`GET /{sideId}/image`）、图片上传（`PUT /{sideId}/image` 原图、`PUT /{sideId}/image/rendered` 成品图、`PUT /{sideId}/image/thumbnail` 缩略图） |
...
| 6 | 校验器 | `src/main/java/com/tightening/service/MissionConfigValidator.java` |
| 7 | Controller x3 | `src/main/java/com/tightening/controller/ProductMissionController.java` 等 |
