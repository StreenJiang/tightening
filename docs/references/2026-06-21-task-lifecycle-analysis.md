...
OperationGuidance 是一个多客户版本的 WinForms 操作引导系统——每个版本面向不同客户独立部署。目前实际在用的版本：标准版、SCII/SCII_XT、GLB、YMT。Task（任务）是核心业务实体，代表一个包含多个螺栓拧紧点位的产品装配任务。

本文档追踪 Task 从被用户选择、激活、执行拧紧、到最终完成（OK 或 NG）的完整生命周期，包括所有异步任务、取消令牌管理、以及各版本的差异化行为。已废弃或不使用的版本（YF/WHYC/TZYX）和多设备独立模式不在本文档范围内。

> **阅读指引**: 文档中使用的专用术语（如 `_activeTaskCts`、`_backgroundTaskCts`、`_checkCts`）详见 §10 取消令牌管理；行号引用格式 `:1234` 指 `AWorkplaceContentPanel.cs` 的行号（除非另有标注）。
...
```
AWorkplaceTaskView<T, V>            (抽象泛型基类, 视图外观层)
  ├── WorkplaceTaskView             (标准版)
  ├── WorkplaceTaskView_SCII        (SCII/SCII_XT 客户版, 批头计数器)
  ├── WorkplaceTaskView_GLB         (GLB 客户版, PLC 集成)
  └── WorkplaceTaskView_YMT         (YMT 客户版, 日聚合导出)
...
- **AWorkplaceTaskView<T,V>**: 外观层（Facade），负责 Task 列表展示、用户选择、创建 Workplace 面板。View 级别的 Dispose 管理 `_checkCts`
...
2. **OnHandleCreated**（异步）：
   ...
   - `ActivateTaskAutomatically()` — 自循环模式自动激活
...
| 状态 | 含义 | 设置位置 |
|---|---|---|
| `UNACTIVATED` | 未激活（初始状态） | 构造时默认; TerminateTask(UNACTIVATED) |
| `ACTIVATED` | 已激活 | ActionAfterActivatingTask |
| `OPERATION_ENABLE` | 允许操作（无锁定） | StartLockCheckingTask（lockMsgs 为空） |
| `OPERATION_DISABLE` | 禁止操作（工具锁定） | StartLockCheckingTask（lockMsgs 非空） |
| `FINISHED_OK` | 完成 - 合格 | TerminateTask(FINISHED_OK) |
| `FINISHED_NG` | 完成 - 不合格 | TerminateTask(FINISHED_NG) |
...
| 字段 | 类型 | 位置 | 含义 |
|---|---|---|---|
| `_activated` | `volatile bool` | AWorkplaceContentPanel:36 | task 是否处于激活状态 |
| `_isActivating` | `bool` | :164 | 正在执行 ActivateTask 流程中 |
| `_pendingBoltBarCodePopup` | `bool` | :165 | 激活完成后需重新打开扫码弹窗 |
| `_needLoosening` | `bool` | :57 | 是否需要反松（NG 后先反松再重拧） |
| `_taskNGAdminConfirmed` | `volatile bool` | :60 | 管理员是否已确认 NG |
| `_exportTriggered` | `int` | :65 | 原子锁，确保导出只触发一次 |

---

## 4. Task 选择与 Workplace 初始化

### 4.1 启动路径

**AWorkplaceTaskView.Initialize(bool operatorOpenning)**:

```
operatorOpenning = true  → 直接进入工作台 (OpenWorkplaceViewDirectly)
operatorOpenning = false → 显示 Task 列表 (OpenTaskListView)
```

> `operatorOpenning` 由 `MainForm` 中 `AfterLogin` 菜单循环根据角色配置传入——管理员通常看到 Task 列表，产线操作工通常直接进入工作台。

### 4.2 Task 列表展示

`OpenTaskListView()` 创建 `TaskListPanel`，包含：
- "选择任务" 标题
- "直接进入工作台" 按钮（跳过选择）
- Task 卡片列表
...
- 取消旧的 `_checkCts`，创建新的
- `FetchDataAsync()` — `Task.Run` 中调用 `QueryProductTaskList` API
- 成功后调用 `_taskListPanel.RefreshTaskBlocks()`
- RefreshTaskBlocks 使用两阶段加载：骨架 UI → 后台异步加载封面图（`SemaphoreSlim(4)` 控制并发）

### 4.3 进入工作台

用户选择 Task → `OpenWorkplaceView(taskId)`:

```
OpenWorkplaceView(taskId)
  ├── Dispose 旧的 _workplacePanel
  ├── 创建 CustomTabPanel (pagePanel)
  ├── 创建 WorkplaceTopBar (V 实例)
  ├── _workplacePanel = GetWrokplacePanel(taskId, topBar)  ← 各版本工厂方法
  ├── topBar.Workplace = _workplacePanel
  ├── pagePanel 添加 topBar + _workplacePanel
  └── 隐藏主面板，显示工作台面板
```

`GetWrokplacePanel` 内部调用 `new WorkplaceContentPanel_XXX(taskId, resetTaskName)`:
1. 查询 `QueryProductTaskDetail` 获取完整 Task 数据
...
| 方式 | 触发路径 |
|---|---|
| **手动扫码激活** | 扫码弹窗 "激活任务" 按钮 → `CheckCanActivateTask()` → `ActivateTask()` |
| **自循环模式** | `ActivateTaskAutomatically()` → 延迟 500ms → `ActivateTask()` |
| **USB 扫码器** | `ActivateTaskAutomatically()` → 打开扫码弹窗 → 扫码完成后激活 |
| **PLC 条码自循环** (GLB) | PLC 读取条码 → `ActionAfterRecevingBarCode()` → 自动填充/激活 |

### 5.2 ActivateTask() 完整步骤链

**文件**: `AWorkplaceContentPanel.cs:1314`

```
ActivateTask()  [public virtual async void]
│
├─ [Step 0] 清理旧的取消令牌
│   ├── 遍历 _backgroundTaskCts → Cancel() → Dispose()
│   ├── _backgroundTaskCts.Clear()
│   ├── _activeTaskCts.Cancel() → Dispose()
│   └── _activeTaskCts = new CancellationTokenSource()
│
├─ [Step 1] PrepareBeforeActivatingTask()           [protected virtual]
...
│
├─ [Step 2] await ValidationBeforeActivatingTask()  [protected virtual async Task<bool>]
│   ├── 检查: 站点配置存在
...
│   ├── _isActivating = true
│   │
│   ├─ [Step 4] InitializeBeforeActivatingTask()    [protected virtual]
...
│   │
│   ├─ [Step 5] await ActionAfterActivatingTask()   [protected virtual async Task]
│   │   ├── 重置扫码计数 (ProductScanCount=0, PartsScanCount=0)
│   │   ├── 创建 _taskRecord（初始 task_result = NG）
│   │   │   └── _apis.AddOrUpdateTaskRecord() — 写入数据库
...
│   ├── 填充 bolt_serial_num, task_record_id, vin_number
...
│   │   └── 全部完成:
│   │       ├── _taskRecord.task_result = OK
│   │       ├── AddOrUpdateTaskRecord()
│   │       ├── 挑战任务检查: AddChallengeResult(MISSION_OK)
│   │       └── TerminateTask(FINISHED_OK)
│   │
│   └── [NG 路径]:
...
> **注意**: `max_ng_num = 0` 时永不触发上限终止（代码条件为 `!= 0`），该值来自 `ProductTaskDTO`，默认值=3。
...
├── bolt.NgTimes++
│
├── if (max_ng_num != 0 && NgTimes >= max_ng_num): ← _task.max_ng_num
│   ├── StoreTighteningData(NG)
│   ├── TerminateTask(FINISHED_NG)
│   └── TaskNGConfirmPopUp("NG次数达上限，任务失败，请输入管理员密码")
...
> **GLB OK 路径**: GLB **完全重写**了 `DoAfterRecevingTighteningDataAsync`（不调用 base），OK 路径逻辑与基类功能等价但实现独立。主要差异：额外维护 `_operationDatasCached` 缓存（供 `StoreTighteningDataToOuterDatabase()` 在 TerminateTask 时批量写入外部数据库）。

**GLB NG 处理** — 逻辑结构与 SCII 一致（max_ng_num → 反松 → password_need_time），但有以下差异：
| 差异点 | SCII | GLB |
|---|---|---|
| `_errorMsg` 赋值 | 无 | ✅ 有（用于外部 DB 上报） |
| `TaskNGConfirmPopUp` | Override：激活蜂鸣器 | 使用基类版本，无蜂鸣器 |
...
### 8.1 TerminateTask 完整步骤

**文件**: `AWorkplaceContentPanel.cs:2819`

```
TerminateTask(status)  [public virtual async Task]
│
├─ [1] 取消所有后台任务（防竞态）
...
│   └── _isRedo = NO
│
├─ [10] await OnTaskCompleted(status)
...
│
└─ [11] 自循环检查
    └── if (非挑战任务 && task_result == OK && 自循环模式):
        └── ActivateTaskAutomatically()
```

> **注意**: NG 完成（task_result != OK）不会触发自循环。只有当 task_result == OK 且自循环模式启用时才会自动重新激活。task_result 的赋值位置为 §6.2 OK 路径末尾（`_taskRecord.task_result = OK`）。

### 8.2 各版本 TerminateTask 覆盖

| 版本 | 覆盖行为 | 位置 |
|---|---|---|
| **SCII/SCII_XT** | `ResetTaskDetails()` + 日志 → `await base.TerminateTask(status)` | SCII.cs:1512 |
| **GLB** | 1. `StoreTighteningDataToOuterDatabase()` — 将缓存在 `_operationDatasCached` 中的全量拧紧数据批量写入外部数据库 2. PLC 同步发送 `SendJobFinished(true)` + `SendJobResult(result)`（`Thread.Sleep` 重试，最多阻塞 ~300ms）3. `await base.TerminateTask(status)` | GLB.cs:56 |
...
```
OnHandleDestroyed:
├── 取消所有 CTS (_activeTaskCts + _backgroundTaskCts)
├── 补充 NG 导出（如果从未触发过导出）
│   └── OnTaskCompleted(FINISHED_NG).Wait() — 同步等待
...
### 8.4 AWorkplaceTaskView.Dispose

**文件**: `AWorkplaceTaskView.cs:142`
...
**启动**: ActionAfterActivatingTask
**周期**: 50ms（finally 块中的 `await Task.Delay(_lockCheckingTaskDelay)` 控制）
**令牌**: `CreateLinkedTokenSource(_activeTaskCts.Token)`
...
**启动**: ActionAfterActivatingTask（仅 `_arrangerNeeded = true` 时）
**周期**: 100ms
...
│   │   └── 已超时:
│   │       ├── 重试次数 < max: 弹窗确认 → 重新发信号
│   │       └── 重试次数 >= max: TerminateTask(FINISHED_NG)
│   └── 全部就位: 移除锁定, _arrangerPositionOk = null
```

### 9.3 StartSetterSelectorTask（套筒选择器检查任务）

**启动**: ActionAfterActivatingTask（仅 `_setterSelectorNeeded = true` 时）
**周期**: 100ms
...
```
字段初始化(AWC:71): _activeTaskCts = new CancellationTokenSource()
ActivateTask:  取消旧的 → Dispose → 创建新的
Start*Task:       LinkedTokenSource 加入 _backgroundTaskCts
TerminateTask: 取消所有 _backgroundTaskCts → 等待 100ms → Dispose
OnHandleDestroyed: 取消 _activeTaskCts + 所有 _backgroundTaskCts
...
| 场景 | 触发位置 | 终止状态 |
|---|---|---|
| 验证失败 | ActivateTask → ValidationBeforeActivatingTask = false | UNACTIVATED |
...
| SCII SkipScrewPoints | ActivateTask 快捷路径 | FINISHED_OK |
...
| 文件 | 行数 | 核心内容 |
|---|---|---|
| `Views/AbstractViews/AWorkplaceTaskView.cs` | ~150 | View 外观层：Task 列表、OpenWorkplaceView、Dispose |
...
| `Views/WorkplaceTaskView_SCII.cs` | ~1700 | SCII/SCII_XT：批头计数器、max_ng_num、SkipScrewPoints |
| `Views/WorkplaceTaskView_GLB.cs` | ~200 | GLB：PLC 信号 + 外部数据库 |
| `Views/WorkplaceTaskView_YMT.cs` | ~30 | YMT：日聚合导出（覆盖 ExportDataAsync） |
...
| `Service/Models/DTOs/ProductTaskDTO.cs` | - | Task DTO（max_ng_num 默认=3） |

> **说明**: 文件索引中 `WorkplaceTaskView_SCII.cs`、`WorkplaceTaskView_GLB.cs`、`WorkplaceTaskView_YMT.cs` 属于 View 外观层（Facade），负责 Task 列表和视图生命周期。各版本的业务逻辑覆盖（批头计数器、PLC 信号、日聚合导出等）实际位于对应的 `WorkplaceContentPanel_SCII`、`WorkplaceContentPanel_GLB`、`WorkplaceContentPanel_YMT` 类中。本文档引用时以源文件中类名标识版本差异，文件命名与类名不完全对应。
...
4. **Token 管理良好**: 所有异步循环都有取消令牌保护，Dispose 路径正确处理。但 `async void` 方法（如 `ActivateTask`）的异常不会传播给调用者
...
### 15.1 `async void ActivateTask()` 异常风险

`ActivateTask()` 没有整体 try-catch。如果 `ActionAfterActivatingTask()`（`:1341`）中抛出异常：

- `_isActivating` 保持 `true`（`:1343` 不会执行）
- `_activated` 保持 `true`（`:1334` 已设置）
- 后台任务（LockChecking/Arranger/SetterSelector）未启动
- Task 进入**永久卡死**状态：后续拧紧数据会被接收，但 `_taskRecord` 可能为 null → 触发 `NullReferenceException`

### 15.2 挑战任务（Challenge Task）前置验证

文档 §7 仅在 OK 路径提到 `AddChallengeResult`，但激活前的完整挑战任务验证路径未覆盖：

- `CheckChallengeTaskConfirmation()`（`:1168`）在激活前验证挑战任务约束（首档岗位检测、前置任务检查、挑战校验项检查）
...
|---|---|---|---|---|
| `ActivateTask` | ✅定义 | 重写(含SkipScrew快捷路径) | 继承 | 继承 |
| `DoAfterRecevingTighteningDataAsync` | ✅定义 | **完全重写** | **完全重写** | 继承 |
| `TerminateTask` | ✅定义 | 前后置+base | 前后置+base | 继承 |
...
当 `SkipScrewPoints = true` 时，SCII 覆盖了 `ActivateTaskAutomatically()`（SCII.cs:167）直接 `return`。这意味着：
- 自循环模式（`IsTaskSelfLoopingModeEnabled`）被完全禁用
- USB 扫码器模式也被跳过
- 每次 task 完成后需要人工手动重新激活

### 15.5 GLB PLC 条码自循环 (ActivateTaskAutomatically)

GLB 覆盖了 `ActivateTaskAutomatically()`（`GLB.cs:77-174`），实现完整的 PLC 条码自循环：弹出用户确认 → `StartReadFromPLC()` 异步循环读取 Siemens PLC → 超时 5 秒 → 收到条码后 `ActionAfterRecevingBarCode(barCode)` 校验并自动激活 → 失败可重试。

### 15.6 `OnHandleDestroyed` 中 `Wait()` 的潜在死锁

`:3006` 行 `OnTaskCompleted(FINISHED_NG).Wait()` 在 UI 线程同步阻塞。如果 `_storeTighteningDataLock` 被另一个操作持有，且该操作的下一步需要 UI 线程（通过 `BeginInvoke`），可能导致死锁。风险较低但存在。

### 15.7 `SendPSet` 快速路径无取消令牌

`:2117` 行的 `task.SendPSetAsync(pset.Value)` 不传入 `_activeTaskCts.Token`。如果工具连接阻塞，task 终止时此调用无法被取消。后续自动重试路径正确传入了令牌。

### 15.8 工具锁策略：两种模式的区别

| 模式 | 配置键 | 策略 | 触发时机 |
|---|---|---|---|
| `IsAutoLockToolEnabled` | 安全锁 | Task **不活跃**时锁工具 | 工具加载后、Task 终止时、Handle 销毁时 |
...
| 位置 | 省略内容 | 代码行 |
|---|---|---|
| §8.1 TerminateTask | Step [7] 与 [8] 之间的扭矩/角度颜色恢复（`_torque.ForeColor = NORMAL; _angle.ForeColor = NORMAL`） | `:2876-2879` |
| §8.1 TerminateTask | Step [1] 的 `if (_backgroundTaskCts.Count > 0)` 守卫条件 | `:2822` |
| §8.1 TerminateTask → Step [10] OnTaskCompleted 内部 | Step [10] 的 `if (!IsExcelExportEnabled && !IsTxtExportEnabled) return` 守卫 | `:2743` |
...
以下方法和流程存在于源代码中，直接影响 task 生命周期，但未在文档主体中描述。

#### `CheckCanActivateTask()`（`ABarCodeInputPopUpForm.cs:679`）

激活前的关键门控方法。在扫码弹窗的"激活任务"点击路径中被调用（AWC:1134），检查：
1. `_task.id > 0` — 必须选择了任务
2. `ProductBarCode` 非空 — 必须录入产品追溯码
3. 物料码数量匹配 — 如有物料码规则，录入数量必须等于预期

失败时高亮空字段并弹出警告。文档 §5.1 的"扫码弹窗 → 激活任务按钮 → ActivateTask()"跳过了这个中间门控。

#### `PopUpTaskListForm()` + `ActionAfterSwitchTask()`（AWC:419, AWC:459）

运行时任务切换的完整流程（不退出工作台）：
1. `PopUpTaskListForm` 弹出"选择任务"弹窗（含 TaskListPanel）
2. 用户选择任务 → `SwitchToTask()` 替换 `_task` 数据、重置条码缓存、刷新 UI
3. `ActionAfterSwitchTask()` → `ActivateTaskAutomatically()`（自循环模式自动激活）
4. SCII 覆盖额外调用 `ResetTaskDetails()` 重置批头计数器
...
#### `BoltNGConfirmPopUp()` vs `TaskNGConfirmPopUp()` 的区别
...
| `TaskNGConfirmPopUp()` | AWC:2309 | 任务级 NG（达 `max_ng_num` 上限），弹出管理员密码窗 | `allowCancel: false` |
| `TaskNGConfirmPopUp()` (SCII override) | SCII:1255 | 同上 + 激活蜂鸣器（`BuzzerController`） | `allowCancel: false` |
