...
**AnengGateway（安维能通信盒）**:
第三方集成通信网关设备。提供力臂专用接口和 IO 信号点，通过 Modbus RTU over TCP 协议与多种子设备通信。一个通信盒可挂接任意数量和类型的子设备。连接/断连/重连由 DeviceManager 管理，子设备可用性由通信盒 handler 内部轮询维护。
_Avoid_: IO Box、IO 盒子、通信模块

**SubDevice（子设备）**:
通过通信盒间接连接的设备的总称。无独立 TCP 连接，不经过 DeviceManager 管理。DB 中通过 `gateway_device_id` 指向所属通信盒。在 DeviceRegistry 中注册为对应的业务接口（IArm / ISetterSelector / IArranger）。
_Avoid_: 网关设备、从设备

**Arm（力臂）**:
通过通信盒力臂专用接口连接的定位辅助设备。支持 CF01~CF04 型号，型号配置存在 `arm_model_config` 表。IArm 接口暴露 `getCurrentCoordinates()` 读取 X/Y/Z 坐标，用于引导工具到位。
_Avoid_: 力臂传感器、定位臂

**SetterSelector（批头选择器/套筒选择器）**:
通过通信盒 IO 信号点控制的套筒切换设备。ISetterSelector 接口暴露 `writePosition(position)` 和 `reset()`。支持 4 位和 8 位型号。
_Avoid_: 套筒切换器、批头切换器

**Arranger（排列机/螺丝机）**:
通过通信盒 IO 信号点控制的螺丝送料设备。IArranger 接口暴露 `sendPulse(channels)` 触发指定通道脉冲送料。8 通道，支持信号位反转配置。
_Avoid_: 送钉机、螺丝排列机、feeder
...
**DeviceStatus（设备连接状态）**:
CONNECTING（连接中）、CONNECTED（已连接，所有功能正常）、DEGRADED（已连接但部分子设备异常，不触发 TCP 重连）、DISCONNECTED（已断开）、NONE（初始无状态）。
_Avoid_: 在线/离线

**DeviceType（设备类型）**:
支持的设备型号枚举。直属工具: ATLAS_PF4000(1)、ATLAS_PF6000_OP(2)、FIT_FTC6(3)、SUDONG_X7(4)；网关及子设备: ANENG_GATEWAY(10)、ARM(11)、SETTER_SELECTOR(12)、ARRANGER(13)。ID 1-9 为直属工具段，10-19 为网关及子设备段。子设备类型不对应 DeviceHandler。
_Avoid_: 设备型号

**DeviceHandler（设备处理器）**:
协议处理器的根接口。工具继承链: DeviceHandler → TCPDeviceHandler → ToolHandler → 具体协议 Handler（AtlasPFSeriesHandler / FitSeriesHandler / SudongSeriesHandler）。网关类设备（如安维能通信盒）直接实现 DeviceHandler，不经过 TCPDeviceHandler（通信模式不同，非请求-响应）。
_Avoid_: 驱动、适配器
...
已废弃。当前方案：不设独立的前置监控器，各 Capability（如 SendArrangerSignal、SendSetterSelector）在执行时自行检查子设备连接状态，不可用时通过 lockReason 机制阻塞任务推进。设备连接状态通过 SSE 实时通知前端。
_Avoid_: 前置检查器
