...
`ProductTaskController.list` 是唯一的分页接口。它调用 `taskService.listByPage()` 返回 MyBatis-Plus `Page<ProductTask>`，但当前只取 `getRecords()` 放入 `ApiResponse`，丢弃了 `total`/`size`/`current` 等分页元数据。前端无法知道总页数，分页组件不可用。
...
### 修改 `ProductTaskController.list`

- **返回类型**：`ResponseEntity<ApiResponse<List<ProductTaskDTO>>>` → `ResponseEntity<ApiResponse<PageResult<ProductTaskDTO>>>`
...
```diff
- data: ProductTaskDTO[]
+ data: { records: ProductTaskDTO[], total: number, size: number, current: number }
```

### 测试更新

`ProductTaskControllerTest` 中涉及 `list` 方法的两个测试用例需更新：
- 返回类型断言适配 `PageResult<ProductTaskDTO>`
...
| `ProductTaskController.list` | 返回类型 + 方法体 | **是**（API 响应格式变更） |
| `ProductTaskControllerTest` | 类型断言更新 | 否 |
