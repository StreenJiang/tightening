...
工作台就绪是**一次性的外部准备**，不在生命周期内部。切换 Task 时需要重新执行。

```
┌────────────────────────────────────────────────────────────────┐
│                                                                │
│  [工作台初始化 — 生命周期外部，一次性 / Task 切换时重执行]       │
│                                                                │
│  选择 Task → 加载 TaskData → 加载设备 → 加载扫码规则 → READY │
│      ↑                                                         │
│      │ (切换 Task: 重新走此流程)                               │
│      │ (自循环: OK 时跳过，Task 不变)                          │
│      │                                                         │
...
| 输入 | 说明 |
|---|---|
| `taskData` | 已加载的 Task 完整数据（含螺栓列表、条码规则、max_ng_num 等），生命周期内不可变 |
...
**关键规则**:
- **切换 Task**：走完整的工作台就绪 + 新生命周期。生命周期进行中禁止切换 Task（触发按钮禁用/不响应）
- **自循环**：Task 不变。**仅正常结束(OK)** 触发自循环，跳过工作台就绪，直接回到触发信号。NG/异常结束 → 生命周期结束，**不自循环**。异常结束显示阻断弹窗，操作员确认后可选重试或退出

Task 生命周期划分为 4 个宏观阶段：

| 阶段 | 职责 | 进入 | 退出 |
|---|---|---|---|
| **VALIDATION** | 设备/工作站配置校验、版本特有检查（批头计数器等）。注：条码校验和挑战任务在触发阶段（§2.3）执行 | 收到触发信号 + CheckCanActivate 通过 | 校验通过 → ACTIVATION；校验失败 → IDLE |
| **ACTIVATION** | 螺栓初始化、排列机/套筒选择器信号、PSet 下发、TaskRecord 创建、后台监控启动 | 校验通过 | 激活完成 → OPERATION；激活失败 → IDLE |
...
**失败路径规则**:
- VALIDATION 失败 → **返回 IDLE**（不解锁/锁定工具取决于 `autoLockToolEnabled`，不导出数据，不创建 TaskRecord）
...
**工作台就绪失败路径**：
- 加载 TaskData 失败（数据损坏/不存在）、设备连接失败 → 显示错误状态，**阻止进入 READY 状态**
...
ACTIVATING 子状态仅执行 `CreateTaskRecord`。螺栓初始化在 PREPARING 中完成；排列机/套筒/PSet 信号移至 OPERATION 的 `SWITCH_BOLT` 子状态（每次螺栓切换时执行）。
...
生命周期触发由三个独立环节组合而成。触发前，工作台必须已处于 READY 状态（Task 数据、设备、扫码规则全部就绪）。
...
每次扫码都会触发校验（校验当前录入的全部条码）。校验策略由 Task 的条码配置决定：
...
CheckCanActivate(context):
    ① task 配置了产品追溯码？ ─── 未录入 → 拒绝，提示扫码
    ② task 配置了物料码？   ─── 未录入 → 拒绝，提示扫码
...
SkipScrewCheck 在触发阶段执行，与 CheckCanActivate 同级。当 Task 配置了 `SkipScrew=true` 时，跳过所有检测、所有流程、所有设备交互，直接生成结果。

```
SkipScrewCheck.execute(ctx):
    if ctx.taskData.skipScrew:
        // 快速通道: 创建 TaskRecord → 进入 FINALIZATION → 返回 Completed(OK)
        1. ctx.shouldSelfLoop = false               // SkipScrew 禁止自循环
        2. CreateTaskRecord(task_result = OK)  // 直接写入 OK
...
- 仅创建一条 `task_result=OK` 的 TaskRecord + 触发 ExportData 导出
...
| 类型 | 说明 | 数据模型 |
|---|---|---|
| `ALL` | 解锁当日此工作站的全部 Task | `inspection_task` 表记录一次检验任务，完成后标记 COMPLETED |
| `MULTIPLE` | 绑定到 N 个特定 Task | `inspection_task` + `task_inspection_binding`（N:N），完成后解除绑定 |

ChallengeTaskCheck 执行逻辑：

```
ChallengeTaskCheck.execute(ctx):
    if !ctx.taskData.inspectionRequired:
        return Pass

    // 三大类检查（具体实现由条码校验模块和岗位管理模块提供，ChallengeTaskCheck 调用对应验证服务获取结果）：
    // 1. 追溯码检查 — 错码/重码检测
    if !barcodeService.validateProductCode(ctx.taskData):
        return Fail("产品追溯码校验失败，存在错码或重码")

    // 2. 物料码检查 — 错码/重码检测
    if !barcodeService.validatePartsCode(ctx.taskData):
        return Fail("物料码校验失败，存在错码或重码")

    // 3. 前置岗位检查 — 前置岗位任务完成状态验证
    if !preShiftChecker.verifyPrecedingStations(ctx.taskData.workstationId):
        return Fail("前置岗位任务未完成，请先完成前置岗位")

    return Pass
```

检验任务使用与常规 Task 相同的 LifecycleEngine 驱动，不重复设计。
...
自循环（`SELF_LOOP`）只是一种**触发信号**，不等于"无条码直接激活"。FINALIZATION 中 `shouldSelfLoop=true` 且 `taskResult=OK` 时发出该信号，然后：
...
└─────────────────────┘  └──────────────────┘
     ... ──► [全部 Pass] → ACTIVATION
     ... ──► [任一 Fail] → IDLE(不导出数据，不创建 TaskRecord)
```
...
ACTIVATING 阶段:
  ┌──────────────┐
  │ CreateTask│  TaskRecord 创建（task_result = NG）
  │ Record       │
  └──────────────┘
     ... ──► ACTIVATED → [→OPERATION]
...
**设计要点**：
- CreateTaskRecord 是 ACTIVATION 管道的**唯一** Capability——螺栓排列机/套筒/PSet 信号下发移至 OPERATION 的 `SWITCH_BOLT` 子状态（每次螺栓切换时执行）
...
AdvanceBolt.execute(ctx):
    ...
    // 尝试推进到下一螺栓
    nextIndex = nextBoltIndex(ctx.currentBoltIndex, ctx.currentSideIndex, ctx.taskData.bolts)
    if nextIndex != null:
...
- **状态标记**：当前螺栓标记为 `DONE` 后，`determineTaskResult` 根据所有螺栓状态集合判断整体结果。
...
lockMsgs 的写入者（OPERATION 阶段内活跃，详见《task-lifecycle-analysis.md》§9.1）：
...
SelfLoopCheck Capability 在 EXPORTING 子状态的 ExportData 之后执行，检查 `taskResult == OK` 且 `shouldSelfLoop == true`：两者均满足时引擎在 Completed 处理中投递 SelfLoopSignal；否则直接结束生命周期，不自循环。
...
│  ConfigCheck  │  CreateTask│  Signal       │ LockTools    │
...
TaskContext {
    // === 不可变部分（生命周期外部注入） ===
    taskData       // 已加载的 Task 完整数据（含螺栓配置列表、产品信息、max_ng_num 等）
                      //   taskData 业务参数: max_ng_num(NG次数上限), password_need_time(第几次NG起需管理员密码)
                      //   taskData.bolts[i] 包含: boltId, torque_min/max, angle_min/max,
...
onMessage(msg, context):
    switch msg:
        ...
        case SelfLoopSignal:
            if shouldSelfLoop && context.taskRecord != null && context.taskRecord.taskResult == OK:
...
handleStageResult(result, context):
    switch result:
        case Advance(nextStage):
            if nextStage == FINALIZATION:
                ...
                if context.taskRecord != null:
                    context.taskRecord.taskResult = determineTaskResult(context)
        ...
        case Interrupted(reason):
            ...
            if context.taskRecord != null:
                context.taskRecord.taskResult = FINISHED_NG
            finalResult = executeStage(FINALIZATION, context)
            handleStageResult(finalResult, context)
        case Completed(lifecycleResult):
            if shouldSelfLoop && lifecycleResult.taskResult == OK:
                inbox.enqueue(SelfLoopSignal)
            // 生命周期结束，Actor 等待下一次 ActivateTask
            // NG/Interrupt: 不自循环，直接结束
        case Aborted(reason, details):
            // 验证/激活失败，任务未启动——无 TaskRecord，无导出
...
start(context):
    // 被 ActivateTask 消息触发。启动后由 handleStageResult 驱动各阶段推进。
    → context.cancellationToken = new CancellationToken()
...
executeStage(stage, context):
    ...
    switch transition.target:
        ...
        case ALL_BOLTS_DONE:
            ...
            // taskResult 由 handleStageResult(Advance(FINALIZATION)) 中
            // 调用的 determineTaskResult 根据全部螺栓状态统一判定
            return Advance(FINALIZATION)
        case RELEASING:
            // 终点子状态——不执行管道 Capability，直接返回 Completed
            // SelfLoopCheck 等收尾检查已在 EXPORTING 子状态管道中执行
            // taskRecord 为空防御（极端中断路径下可能未创建）
            result = context.taskRecord != null ? context.taskRecord.taskResult : FINISHED_NG
...
computeNgDecision(context):
    ...
    if boltState.ngTimes >= context.taskData.maxNgNum:
        return TERMINATE
    return RETRY

// === 整体任务结果判定（进入 FINALIZATION 前调用） ===

determineTaskResult(context):
    // 根据所有螺栓的最终状态计算整体任务结果
    // 在 Advance(FINALIZATION) 路径中被调用（Interrupted 路径直接设为 FINISHED_NG），
    // 结果写入 context.taskRecord.taskResult，供 SelfLoopCheck 和 Completed 使用
    // 规则:
...
determineTransition(stage, subState, pipelineResult, context):
    ...
    //     判定逻辑: boltStates[currentBoltIndex].ngTimes >= taskData.maxNgNum
...
TorqueRangeCheck.execute(ctx):
    bolt = ctx.taskData.bolts[ctx.currentBoltIndex]
...
SendArrangerSignal.execute(ctx):
    arranger = ctx.deviceRegistry.getArranger(ctx.taskData.bolts[ctx.currentBoltIndex].arrangerId)
...
ExportData Capability:
    execute(ctx):
        task = ExportTask(
            type = "standard_excel",
            taskRecordId = ctx.taskRecord.id,
            payload = serialize(ctx.taskRecord, ctx.tighteningData),
            status = PENDING
        )
...
StoreToOuterDB Capability:
    execute(ctx):
        task = ExportTask(
            type = "outer_db_store",
            payload = serialize(ctx.taskRecord, ...),
            status = PENDING
        )
...
**等待期间的行为**：工具被 LockStateMonitor 锁定，操作工无法拧紧。TighteningData 不会到达（工具锁着不能拧）。持久监控照常运行。操作员可手动终止 Task。
...
CREATE TABLE export_task (
    ...
    task_record_id INTEGER NOT NULL,      // 关联的 TaskRecord ID
    ...
    FOREIGN KEY (task_record_id) REFERENCES task_record(id)
);
```
...
ExportData.execute(ctx):
    // taskRecord 空值防御（极端路径下可能未创建）
    if ctx.taskRecord == null:
        log("ExportData: taskRecord 为空，跳过导出")
        return Skip("taskRecord 为空")
    task = ExportTask(
        type = "standard_excel",
        taskRecordId = ctx.taskRecord.id,
        payload = jsonSerialize({
            taskId: ctx.taskData.id,
            taskRecordId: ctx.taskRecord.id,
            tighteningDataIds: ctx.tighteningData.map(d => d.id),
            exportFormat: ctx.taskData.exportConfig.format
        }),
        status = PENDING
    )
...
**关键设计决策**：
- `export_task` 和 `task_record` 在同一 SQLite 数据库中，保证事务一致性
...
验证失败→IDLE，不进入 FINALIZATION，不导出数据，不创建 TaskRecord。工具锁定行为由 §2.1 的 `autoLockToolEnabled` 配置决定。
...
激活失败→IDLE，不进入 FINALIZATION。CreateTaskRecord 是 ACTIVATING 管道的最后一个 Capability，因此激活失败不会产生 TaskRecord。
...
| **任务 NG** | 任务整体不合格 — `FINISHED_NG` | `taskRecord.taskResult` | NG 达上限等导致 |
...
| 层次 | 待考虑事项 | 说明 |
|---|---|---|
| 传输安全 | HTTPS | 明文 HTTP 下 Task 激活/终止可被中间人篡改 |
| 认证 | 请求必须携带身份凭据 | JWT / API Key / mTLS — 需根据部署环境选择 |
| 授权 | 不同命令需要不同角色 | 操作员角色可以激活，管理员角色才能终止/确认 |
| 审计 | 所有 HTTP 触发的操作记录操作者身份 | 与 `taskRecord` 关联 |
...
| 身份传递 | InboundCommand 应携带身份上下文 | `ActivateTask { identity: IdentityContext }` 等 |
...
| `IsTaskSelfLoopingModeEnabled` | `shouldSelfLoop` | 自循环开关（仅 OK 触发自循环） |
| `ActivateTaskAutomatically()` | `SELF_LOOP` 触发信号 | 实现方式 vs 抽象概念 |
| `_activeTaskCts` | `context.cancellationToken` | 具体实现 vs 抽象概念 |
| `CheckCanActivateTask()` | `CheckCanActivate` | 激活前置条件门控 |
| `TerminateTask()` | FINALIZATION 阶段 + `interrupt()` | 过程化调用 vs 声明式阶段 |
...
| `BoltNGConfirmPopUp` / `TaskNGConfirmPopUp` | `AdminConfirm` Capability（复用同一个） | 管理员密码确认（lockMsg 阻塞） |
