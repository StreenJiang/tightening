...
`GET /api/tasks/check-name?name=xxx&excludeId=yyy`
...
在现有 `GET /api/tasks` 加可选参数 `name`：

```
GET /api/tasks?page=1&size=100&name=xxx
```
...
新增 Flyway 迁移，为 `product_task.name` 加部分唯一索引（只对未删除记录）：
...
| 层 | 文件 | 改动 |
|---|---|---|
| Migration | 新增 `V1.0.16__add_task_name_unique.sql` | 部分唯一索引 |
| Controller | `ProductTaskController.java` | 新增 `checkName`；`list` 加 `name` 参数；`create`/`update` 加异常捕获 |
| Service | `ProductTaskService.java` | 新增 `isNameDuplicate`；`listByPage` 加 `name` 参数 |
