...
1. Mock `barCodeMatchingRuleService.listByMissionId(1L)` 返回一条 `PRODUCT_TRACE` 规则
2. Mock `ITool`（id、type），注册到 deviceRegistry（避免 WorkstationConfigCheck 失败）
3. `factory.createEngine(mission, [bolt], {1L: tool}, false, null, null)` — productCode 为 null
...
## §2: 删除 `MissionOrchestrator.startMission()`

**文件**: `src/main/java/com/tightening/lifecycle/MissionOrchestrator.java`

删除整个方法（约 44 行）。

**测试**: `MissionOrchestratorTest.java` — 3 个 `startMission()` 测试改为 `trigger()` 或删除（按 Stage 5 场景覆盖，trigger 已有足够测试覆盖）。

---

## §3: 删除 `InboundCommand.ActivateMission` 及 handler

**生产代码:**
- `InboundCommand.java`: 删除 `ActivateMission` record，清理不再需要的 import（BoltDeviceBinding、ProductBolt、ProductMission、ProductSide、List）
- `LifecycleEngine.java`: 删除 handler 注册行 + 删除 `handleActivateMission` 方法 + 内部方法重命名 `handleActivateMissionInternal`→`startNormalLifecycle`、`handleActivateMissionSkipScrew`→`startSkipScrewLifecycle`

**测试:**
- `LifecycleEngineTest.java`: `ActivateMission` 测试改为 `TriggerRequest`
- `InboundMessageTest.java`: 删除 `ActivateMission` record 测试
...
- `MissionContext.java` — 删除 `tighteningStatus` 字段
...
- [ ] `grep -rn "startMission\|ActivateMission\|SelfLoop\|ControllerStatusCheck" src` 无残留
...
| 问题 | 修复 |
|---|---|
| 测试线程泄漏 | `LifecycleEngineFactoryTest` 加 `@AfterEach` shutdown + 去除无用 `missionRecordService` mock |
...
| 内部方法残留 `ActivateMission` 命名 | `handleActivateMissionInternal`→`startNormalLifecycle`、`handleActivateMissionSkipScrew`→`startSkipScrewLifecycle` |

---

## §7: 修复 SkipScrew 自循环泄漏

**缺陷**: `MissionOrchestrator.trigger()` 的 `onCompleted` 闭包用 `shouldSelfLoop`（settings 值）判断是否自循环，忽略了引擎 `ctx.setShouldSelfLoop(false)` 的决定。

**文件**: `src/main/java/com/tightening/lifecycle/MissionOrchestrator.java` — trigger() 方法 onCompleted 闭包
