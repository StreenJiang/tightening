...
```bash
curl -X POST http://localhost:8080/api/tasks/2/prerequisites \
  -H "Content-Type: application/json" \
  -d '{"prerequisiteTaskId":1,"prerequisiteType":1}'
# 预期: 200, "ok"
...
```bash
curl http://localhost:8080/api/tasks/2/prerequisites
# 预期: 200, JSON 数组
```
...
```bash
curl -X DELETE http://localhost:8080/api/tasks/2/prerequisites/1
# 预期: 200, "ok"
```
...
```bash
# 先建 A→B
curl -X POST http://localhost:8080/api/tasks/1/prerequisites \
  -H "Content-Type: application/json" \
  -d '{"prerequisiteTaskId":2,"prerequisiteType":1}'
# 预期: 200
...
curl -X POST http://localhost:8080/api/tasks/1/prerequisites \
  -H "Content-Type: application/json" \
  -d '{"prerequisiteTaskId":2,"prerequisiteType":3}'
...
```bash
curl -X POST http://localhost:8080/api/tasks/3/inspection-bindings \
  -H "Content-Type: application/json" \
  -d '{"boundTaskId":2}'
# 预期: 200, "ok"
```
...
```bash
curl http://localhost:8080/api/tasks/3/inspection-bindings
# 预期: 200, JSON 数组
```
...
```bash
curl -X POST http://localhost:8080/api/tasks/1/barcode-rules \
  -H "Content-Type: application/json" \
  -d '{
    "name": "追溯码规则",
    "productTaskId": 1,
...
# 预期: 500, "该 task 已存在 PRODUCT_TRACE 规则，每个 task 最多一条"
```

### 4.4 key_char 长度校验（应报错）

```bash
curl -X POST http://localhost:8080/api/tasks/2/barcode-rules \
  -H "Content-Type: application/json" \
  -d '{
    "name": "错误规则",
    "productTaskId": 2,
...
```bash
curl http://localhost:8080/api/tasks/1/barcode-rules
# 预期: 200, JSON 数组
```
...
```bash
curl -X DELETE http://localhost:8080/api/tasks/1/barcode-rules/1
# 预期: 200, "ok"
```
...
```bash
curl "http://localhost:8080/api/sides?taskId=1"
# 预期: 200, JSON 数组
```
...
```bash
curl -X POST "http://localhost:8080/api/bolts?taskId=1" \
  -H "Content-Type: application/json" \
  -d '{
    "productSideId": 1,
    "boltSerialNum": 1,
...
# 预期: 500, "bolt_serial_num 1 在当前 task 中已存在"
```
...
echo "=== 1. Task CRUD ==="
MID1=$(create POST "$BASE/api/tasks" '{"name":"集成测试","maxNgCount":3,"enabled":1,"isInspection":0}')
MID2=$(create POST "$BASE/api/tasks" '{"name":"集成测试2","maxNgCount":5,"enabled":1,"isInspection":0}')
MID3=$(create POST "$BASE/api/tasks" '{"name":"点检","maxNgCount":1,"enabled":1,"isInspection":1,"inspectionScope":2}')

check 200 GET "$BASE/api/tasks/$MID1"
check 200 GET "$BASE/api/tasks?page=1&size=10"
check 200 PUT "$BASE/api/tasks/$MID1" '{"name":"集成测试-改","maxNgCount":5,"enabled":1,"isInspection":0}'

echo "=== 2. 前置依赖 ==="
check 200 POST "$BASE/api/tasks/$MID2/prerequisites" "{\"prerequisiteTaskId\":$MID1,\"prerequisiteType\":1}"
check 500 POST "$BASE/api/tasks/$MID1/prerequisites" "{\"prerequisiteTaskId\":$MID2,\"prerequisiteType\":1}"

echo "=== 3. 点检绑定 ==="
check 200 POST "$BASE/api/tasks/$MID3/inspection-bindings" "{\"boundTaskId\":$MID2}"
check 500 POST "$BASE/api/tasks/$MID3/inspection-bindings" "{\"boundTaskId\":$MID3}"

echo "=== 4. Side CRUD ==="
SID=$(create POST "$BASE/api/sides" "{\"name\":\"测试面\",\"productTaskId\":$MID1}")
check 200 GET "$BASE/api/sides/$SID"
check 200 GET "$BASE/api/sides?taskId=$MID1"
check 404 GET "$BASE/api/sides/99999/image?type=rendered"

echo "=== 5. Bolt CRUD ==="
BID=$(create POST "$BASE/api/bolts?taskId=$MID1" "{\"productSideId\":$SID,\"boltSerialNum\":1,\"torqueMin\":5.0,\"torqueMax\":25.0}")
check 500 POST "$BASE/api/bolts?taskId=$MID1" "{\"productSideId\":$SID,\"boltSerialNum\":1}"
check 200 GET "$BASE/api/bolts?sideId=$SID"

echo "=== 6. 条码规则 ==="
RID=$(create POST "$BASE/api/tasks/$MID1/barcode-rules" "{\"name\":\"追溯码\",\"productTaskId\":$MID1,\"ruleType\":1}")
check 500 POST "$BASE/api/tasks/$MID1/barcode-rules" "{\"name\":\"第二个\",\"productTaskId\":$MID1,\"ruleType\":1}"

echo "=== 7. 级联删除 ==="
check 200 DELETE "$BASE/api/bolts/$BID"
check 200 DELETE "$BASE/api/sides/$SID"
check 200 DELETE "$BASE/api/tasks/$MID1"
check 200 DELETE "$BASE/api/tasks/$MID2"
check 200 DELETE "$BASE/api/tasks/$MID3"
