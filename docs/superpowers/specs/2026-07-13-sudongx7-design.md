...
- `vin` — LifecycleEngine 从 `TaskContext.productCode` 回填
- `parameterSet` — LifecycleEngine 从 `TaskContext.currentPSet` 回填
...
void handleTighteningData(InboundMessage msg, TaskContext ctx, LifecycleEngine engine) {
    ...
    // 不回传 vin 的协议 → 从 TaskContext 回填产品条码
    if (data.getVin() == null || data.getVin().isEmpty()) {
        data.setVin(ctx.getProductCode());
    }
    // 不回传 parameterSet 的协议 → 从 TaskContext 回填
...
其中 `TaskContext.currentPSet` 由 `SendPSet` Capability 在切换成功后写入：
...
| `lifecycle/TaskContext.java` | 新增 `currentPSet` volatile 字段；`previousOperationData`/`pendingCurveData`/`extras` 标记 `@Deprecated` |
...
| 12 | `TaskContext.currentPSet` | 编译通过 |
